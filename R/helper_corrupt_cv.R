...
KOMAP_corrupt_cv <- function(input.cov.train, input.cov.valid, is.wide = TRUE, target.code, target.cui, nm.disease,
  ...
  out_main = KOMAP.est.corrupt.cv(input.cov.train, input.cov.valid, target.code, target.cui, nm.disease,
...
KOMAP.est.corrupt.cv <- function(input.cov.train, input.cov.valid, target.code, target.cui, nm.disease,
                              nm.utl, nm.corrupt.code, nm.corrupt.cui, nm.multi, dict,
                              codify.feature, cuisearch.feature){
  out.all = c()
  ...
  out = gen.KOMAP.est.table.corrupt.cv(input.cov.train, input.cov.valid, nm.disease, nm.utl, nm.corrupt.code, nm.corrupt.cui, nm.multi, target.code, target.cui, dict,
...
gen.KOMAP.est.table.corrupt.cv <- function(input.cov.train, input.cov.valid, nm.disease, nm.utl, nm.corrupt.code, nm.corrupt.cui, nm.multi, target.code, target.cui, dict,
  ...
  for(i in 1:length(out)){
    ...
    if(length(target) == 1){
      ...
      feat.cov.part.all = (feat.cov.train[all.nm, all.nm] + feat.cov.valid[all.nm, all.nm])/2

      U = svd(feat.cov.part.all)
      U = t(U$u %*% sqrt(diag(U$d)))
      colnames(U) = rownames(U) = colnames(feat.cov.part.all)

      # U = U[!rownames(U) %in% nm.corrupt.code, ]
      #! reg1 = stats::lm(U[, target] ~ 0 + U[, 1] + U[, nm.utl])
      reg1 = stats::lm(U[, target] ~ 0 + U[, nm.utl])
      U_new_all = U; U_new_all[, target] = reg1$residuals
      U_new_all[, nm.corrupt.code] = U_new_all[, nm.corrupt.code] - reg1$coefficients * U_new_all[, nm.utl]
      cor(U_new_all[, target], U_new_all[, nm.corrupt.code])
      ...
      for(cv.i in 1:2){
        if(cv.i == 1){
          ...
          U = svd(feat.cov.part.valid)
          U = t(U$u %*% sqrt(diag(U$d)))
          U.valid = svd(feat.cov.part)
          U.valid = t(U.valid$u %*% sqrt(diag(U.valid$d)))
        }
        colnames(U) = rownames(U) = colnames(U.valid) = rownames(U.valid) = colnames(feat.cov.part)
        U_new = U;
        U_new[, target] = U_new[, target] - alpha * U_new[, nm.utl]
        U_new[, nm.corrupt.code] = U_new[, nm.corrupt.code] - alpha * U_new[, nm.utl]
        cor(U_new[, target], U_new[, nm.corrupt.code])

        U.valid[, target] = U.valid[, target] - alpha * U.valid[, nm.utl]
        U.valid[, nm.corrupt.code] = U.valid[, nm.corrupt.code] - alpha * U.valid[, nm.utl]
        ...
        lambda.df.cv.i = data.frame(`lambda` = lambda_vec, `mse` = mse_vec)
        if(cv.i == 1){
          lambda.df = lambda.df.cv.i
        }else{
          lambda.df = full_join(lambda.df, lambda.df.cv.i, by = 'lambda')
        }
      }
      lambda.df$mse = (lambda.df$mse.x + lambda.df$mse.y) / 2
      plot(log(lambda_vec), lambda.df$mse, type = 'l', main = paste(nm.disease, ' Corrupt ICDonly, ', target))
      lines(log(lambda_vec), lambda.df$mse.x, col = 'grey')
      lines(log(lambda_vec), lambda.df$mse.y, col = 'grey')
      abline(v = log(lambda_vec[which.min(lambda.df$mse)]), col = 'red')
      # plot(cv.fit, main = paste(nm.disease, ' Corrupt ICDonly, ', target))
      # plot_data = c(plot_data, list(cv.fit))
      plot_data = c(plot_data, list(lambda.df))


      Y = U_new_all[, target]
      id_target = which(colnames(U_new_all) == target)
      id_target_corrupt = which(colnames(U_new_all) == nm.corrupt.code)
      set.seed(10101)
      model.fit <- glmnet::glmnet(U_new_all[, -id_target_corrupt], Y, intercept = F, alpha = alpha.glmnent, standardize = F,
                                  lambda = lambda.df$lambda[which.min(lambda.df$mse)])
      ...
      for(j in 1:length(target)){
        ...
        U_new_all[, target.j] = reg1$residuals
        nm.corrupt = nm.corrupt.vec[j]
        U_new_all[, nm.corrupt] = U_new_all[, nm.corrupt] - reg1$coefficients * U_new_all[, nm.utl]
        cor(U_new_all[, target.j], U_new_all[, nm.corrupt])
      ...
      for(j in 1:length(target)){
        target.j = target[j]
        id_target = which(colnames(feat.cov.part) == target.j)
        id_target_corrupt = which(colnames(feat.cov.part) == nm.corrupt.vec[j])
        id_elsetarget_corrupt = which(colnames(feat.cov.part) == nm.corrupt.vec[3-j])
        lambda.df = c()
        for(cv.i in 1:2){
          if(cv.i == 1){
            U = svd(feat.cov.part)
          ...
          colnames(U) = rownames(U) = colnames(U.valid) = rownames(U.valid) = colnames(feat.cov.part)
          U_new = U;
          U_new[, target.j] = U_new[, target.j] - alpha[j] * U_new[, nm.utl]
          U_new[, nm.corrupt.vec[j]] = U_new[, nm.corrupt.vec[j]] - alpha[j] * U_new[, nm.utl]
          cor(U_new[, target.j], U_new[, nm.corrupt.vec[j]])

          U.valid[, target.j] = U.valid[, target.j] - alpha[j] * U.valid[, nm.utl]
          U.valid[, nm.corrupt.vec[j]] = U.valid[, nm.corrupt.vec[j]] - alpha[j] * U.valid[, nm.utl]
          cor(U.valid[, target.j], U.valid[, nm.corrupt.vec[j]])
        ...
        lambda.df$mse = (lambda.df$mse.x + lambda.df$mse.y) / 2
        plot(log(lambda_vec), lambda.df$mse, type = 'l', main = paste(nm.disease, ' Corrupt ICDNLP, ', target.j))
        lines(log(lambda_vec), lambda.df$mse.x, col = 'grey')
        lines(log(lambda_vec), lambda.df$mse.y, col = 'grey')
        abline(v = log(lambda_vec[which.min(lambda.df$mse)]), col = 'red')
        # plot(cv.fit, main = paste(nm.disease, ' Corrupt ICDonly, ', target))
        ...
        Y = U_new_all[, target.j]
        model.fit <- glmnet::glmnet(U_new_all[, -c(id_target_corrupt, id_elsetarget_corrupt)], Y, intercept = F, alpha = alpha.glmnent, standardize = F,
                                    lambda = lambda_vec[which.min(mse_vec)])

        # score0 = U_new[, -c(id_target, id_elsetarget_corrupt)] %*% model.fit$beta
        score0 = U_new_all[, -c(id_target_corrupt, id_elsetarget_corrupt)] %*% model.fit$beta
